...
<template>
  <div class="selector">
    <el-input v-model="inputValue" disabled>
      ...
      placement: 'auto'
    }" :width="350" popper-class="pure-popper" trigger="click" @before-enter="onBeforeEnter"
          ...
          <el-input v-model="filterValue" placeholder="搜索" class="px-2 pt-2" clearable />

          <el-tabs v-model="currentActiveType" @tab-click="handleClick">
            <el-tab-pane v-for="(pane, index) in tabsList" :key="index" :label="pane.label" :name="pane.name">
              <el-scrollbar height="220px">
                <ul class="flex flex-wrap px-2 ml-2">
                  <li v-for="(item, key) in pageList" :key="key" :style="iconItemStyle(item)" :title="item"
                    class="icon-item p-2 cursor-pointer mr-2 mt-1 flex justify-center items-center border border-[#e5e7eb]"
                    @click="onChangeIcon(item)">
                    <IconifyIconOnline :icon="currentActiveType + item" height="20px" width="20px" />
                  </li>
                </ul>
                <el-empty v-show="pageList.length === 0" :description="`${filterValue} 没有数据`" :image-size="60" />
              </el-scrollbar>
            </el-tab-pane>
          </el-tabs>

          <div class="w-full h-9 flex items-center overflow-auto border-t border-[#e5e7eb]">
            <el-pagination :current-page="currentPage" :page-size="pageSize" :pager-count="5" :total="totalPage"
              background class="flex-auto ml-2" layout="pager" small @current-change="onCurrentChange" />
