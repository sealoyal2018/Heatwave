...
<template>
  <el-select :filter-method="filterMethod" :model-value="inputValue" :placeholder="props.placeholder" clearable
    filterable popper-class="pure-animate-popper" @clear="onClear">
    <template #empty>
      <div class="w-[280px]">
        <el-scrollbar :view-style="{ overflow: 'hidden' }" class="border-t border-[#e5e7eb]" height="212px" noresize>
          <ul class="flex flex-wrap justify-around mb-1">
            <li v-for="(animate, index) in animatesList" :key="index" :class="animateClass"
              :style="animateStyle(animate)" @click="onChangeIcon(animate)" @mouseenter.prevent="onMouseEnter(index)"
              @mouseleave.prevent="onMouseleave">
              <h4 :class="[
                  `animate__animated animate__${animateMap[index]?.loading
          ...
          <el-empty v-show="animatesList.length === 0" :description="`${searchVal} layout.noData`" :image-size="60" />
        </el-scrollbar>
      </div>
    </template>
  </el-select>
